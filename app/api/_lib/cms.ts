...
// Resolve e valida o token de admin via Neon Auth.
...
const jwks = neonAuthBaseUrl
  ? createRemoteJWKSet(new URL(`${neonAuthBaseUrl}/.well-known/jwks.json`))
  : null;
...
// Resolve contexto de admin com email e papel, validando sessão Neon.
export async function requireAdminContextFromRequest(request: NextRequest) {
  try {
    if (!jwks) {
      return {
        context: null,
        error: jsonError('Configuração de autenticação admin em falta.', 500),
      };
    }
    ...
    return {
      context: null,
      error: jsonError('Não foi possível validar a sessão Neon.', 401),
    };
  }
}
...
export async function listGalleryMedia(scope: 'public' | 'admin') {
  ...
  try {
    ...
    if (scope === 'public' && isPublicDbQuotaExceededError(error)) {
      ...
      console.warn('Gallery query unavailable due to Neon quota; using fallback data.');
    } else {
