...
import { getAdminAccessToken, neonAuth } from '@/lib/neon-auth';
...
export default function BackofficePage() {
  ...
  const authHeaders = useCallback(async () => {
    ...
    if (!token) throw new Error('Sessão Neon expirada.');
    ...
    async <T,>(path: string, init?: RequestInit) => {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          throw new Error('Sessão Neon expirada.');
        }
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        const sessionResult = await neonAuth.adapter.getSession();
        ...
        if (error instanceof Error && (error.message.includes('Sessão Neon expirada') || error.message.includes('401'))) {
          await neonAuth.adapter.signOut().catch(() => undefined);
          router.replace('/backoffice/login');
          return;
        }
    ...
    void bootstrap();
  }, [fetchAdminEndpoint, refreshAll, refreshGovernance, refreshLayout, refreshGallery, router]);
    ...
    <main className="mx-auto min-h-[70vh] w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-stone-600">Login Neon + CRUD completo em todas as secções.</p>
        </div>
        <button
          type="button"
          onClick={async () => {
